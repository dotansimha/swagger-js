...
var lodashFull = require('lodash');

var helpers = require('../helpers');
var Model = require('./model');
var SwaggerHttp = require('../http');
var Q = require('q');
var getActualObjectWithEncryption = require("../string-encryption");
      ...
      if (param.in === 'path') {
        ...
        if (typeof param.collectionFormat !== 'undefined') {
          ...
          if (Array.isArray(qp)) {
            ...
            querystring += param.name + '=' + this.encodeQueryParam(args[param.name]);
          }
        } else {
          querystring += param.name + '=' + this.encodeQueryParam(args[param.name]);
        }
...
Operation.prototype.prettyPrint = function(obj) {
  var method = obj.method || 'Unkown Method';
  var url = obj.url || 'Unknown URL';
  var headers = obj.headers || {};
  var proxy = obj.proxy ? " (Proxy: " + obj.proxy + ")" : '';
  var body = lodashFull.cloneDeep(obj.body) || '(Empty body)';

  console.log("[SwaggerClient] Outgoing network: " + method + " " + url + proxy);

  for (var headerName in headers) {
    console.log("[SwaggerClient] " + headerName + ": " + headers[headerName]);
  }

  getActualObjectWithEncryption(body, true);

  console.log("[SwaggerClient] ", body);
};
...
Operation.prototype.execute = function (arg1, arg2, arg3, arg4, parent) {
  ...
  if (opts.mock === true) {
    ...
    this.prettyPrint(obj, opts);

    getActualObjectWithEncryption(obj.body, false);  
